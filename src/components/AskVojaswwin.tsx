...
export default function AskVojaswwin() {
  ...
  const sendMessage = useCallback(async () => {
    ...
    try {
      ...
      if (!apiKey) {
        setMessages((prev) => [
          ...prev,
          { role: 'assistant', content: 'Oops — the Gemini API key isn\'t configured yet. Please set `VITE_GEMINI_API_KEY` in the `.env` file.' },
        ]);
        setIsLoading(false);
        return;
      }
      ...
      setMessages((prev) => [
        ...prev,
        { role: 'assistant', content: 'Something went wrong connecting to Gemini. Please try again in a moment!' },
      ]);
    } finally {
      setIsLoading(false);
    }
  ...
  const renderContent = (text: string) => {
    ...
    return lines.map((line, i) => {
      ...
      // Inline code
      processed = processed.replace(/`(.*?)`/g, '<code class="bg-black/10 px-1 py-0.5 rounded text-xs">$1</code>');
  ...
  return (
    <>
      ...
      >
        <div className="flex flex-col h-full rounded-t-2xl sm:rounded-2xl border border-black/[0.12] bg-black/80 backdrop-blur-xl shadow-[0_24px_80px_rgba(0,0,0,0.7),0_0_0_1px_rgba(255,255,255,0.05)] overflow-hidden">
          {/* Header */}
          <div className="flex items-center justify-between px-5 py-4 border-b border-black/[0.08]">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-[11px] text-black/40 font-medium">AI-powered • Professional queries only</p>
              </div>
            </div>
            <button
              onClick={() => setIsOpen(false)}
              className="p-2 rounded-xl text-black/50 hover:text-black hover:bg-black/[0.08] transition-all duration-200"
          ...
          <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4 scrollbar-thin">
            {messages.length === 0 && (
              <div className="flex flex-col items-center justify-center h-full text-center px-6 gap-4 opacity-60">
                ...
                <div>
                  <p
                    className="text-base font-bold text-black/80 mb-2"
                    style={{ fontFamily: "'League Spartan', sans-serif" }}
                  >
                    Hey, I'm Vojaswwin! 👋
                  </p>
                ...
                <div className="flex flex-wrap gap-2 mt-2 justify-center">
                  {[
                    ...
                    <button
                      ...
                      className="text-[11px] px-3 py-1.5 rounded-full border border-black/10 bg-black/[0.04] text-black/50 hover:text-black/80 hover:bg-black/[0.08] hover:border-black/20 transition-all duration-200"
                    >
                      {q}
                    </button>
                  ))}
              ...
              >
                <div
                  className={[
                    ...
                    msg.role === 'user'
                      ? 'bg-red-600/90 text-black rounded-br-md'
                      : 'bg-black/[0.07] border border-black/[0.08] text-black/90 rounded-bl-md',
                  ].join(' ')}
                >
            ...
            {isLoading && (
              <div className="flex justify-start">
                <div className="bg-black/[0.07] border border-black/[0.08] rounded-2xl rounded-bl-md px-4 py-3 flex items-center gap-1.5">
                  <span className="w-2 h-2 rounded-full bg-black/ animate-[typingDot_1.4s_ease-in-out_infinite]" />
                  <span className="w-2 h-2 rounded-full bg-black/ animate-[typingDot_1.4s_ease-in-out_0.2s_infinite]" />
                  <span className="w-2 h-2 rounded-full bg-black/ animate-[typingDot_1.4s_ease-in-out_0.4s_infinite]" />
                </div>
              </div>
            )}
          ...
          <div className="px-4 pb-4 pt-2 border-t border-black/[0.06]">
            <div className="flex items-end gap-2 rounded-xl border border-black/[0.1] bg-black/[0.04] px-3 py-2 focus-within:border-black/20 transition-colors">
              <textarea
                ref={inputRef}
                value={input}
                ...
                rows={1}
                className="flex-1 bg-transparent text-sm text-black placeholder-white/30 outline-none resize-none max-h-24 leading-relaxed"
                style={{ fontFamily: "'League Spartan', sans-serif" }}
                disabled={isLoading}
              />
              <button
                ...
                className={[
                  ...
                  input.trim() && !isLoading
                    ? 'bg-red-600 text-black hover:bg-red-700 shadow-lg shadow-red-600/20'
            ...
            <p className="text-[10px] text-black/20 text-center mt-2">
      ...
      {showTooltip && !isOpen && (
        <div className="fixed bottom-24 right-6 z-[9998] animate-[fadeSlideIn_0.5s_ease-out]">
          <div className="relative bg-black/[0.1] backdrop-blur-xl border border-black/[0.15] rounded-2xl px-4 py-3 shadow-[0_12px_40px_rgba(0,0,0,0.5)] max-w-[220px]">
            <button
              onClick={() => {
                setShowTooltip(false);
                setTooltipDismissed(true);
              }}
              className="absolute -top-2 -right-2 w-5 h-5 rounded-full bg-black/10 border border-black/20 flex items-center justify-center text-black/50 hover:text-black hover:bg-black/ transition-all text-xs"
              aria-label="Dismiss tooltip"
            >
              ×
            </button>
            <p
              className="text-sm font-bold text-black/90 mb-0.5"
              style={{ fontFamily: "'League Spartan', sans-serif" }}
            >
              Hey there! 👋
            </p>
            ...
            <div className="absolute -bottom-2 right-6 w-4 h-4 bg-black/[0.1] border-r border-b border-black/[0.15] rotate-45" />
          </div>
        </div>
      )}
